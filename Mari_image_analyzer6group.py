# -*- coding: utf-8 -*-
"""
Created on Mon Mar 10 18:46:14 2025
...
def analyze_image(p, img):
     ...
     final_contours = {key: value for key, value in final_contours.items() if key >= p[9]}
     return(final_contours)
...
def main(args):
    ...
    for x in files:
        ...
        contourlist = analyze_image(p, img)
        final_contours = filter_contours_by_bitwise_and(final_contours, contourlist, img)
        source_images[x] = []
        for y in contourlist:
            if (y>= score_cutoff) and (y in final_contours):
                source_images[x].append(y)
    del x
    try:
        del y
    except:
    ...
    for x in source_images:
        if (len(source_images[x]) > maxl) or ((len(source_images[x]) == maxl) and (sum(source_images[x])>sum(source_images[name]))):
            maxl = len(source_images[x])
            name = x
    del x, maxl
    ...
    temp = [str(round(x,4)) for x in Colonies["Score"]]
    for x in source_images:
        if x == name:
            continue
        contours = []
        scores = source_images[x]
        scores =  [str(round(x,4)) for x in scores]
        for y in range(len(scores)):
            if scores[y] in temp:
                ind =  temp.index(scores[y])
                contours.append(Colonies["Contour"][ind])
        ...
        src_image = cv2.copyMakeBorder(src_image,top=10, bottom=10,left=10,right=10, borderType=cv2.BORDER_CONSTANT,  value=[255, 255, 255])  
    ...
    del x, mask, source_images, name, temp
    try:
        del scores, contours,ind,src_image, roi
    except:
        pass
    
    img2 = mask_img(img, Colonies["Contour"])
    ...
    del contours_necrosis, mask, mask2, contour, i, overlap_mask
    ...
    cv2.imwrite(path+'Group_analysis_results.jpg', np.vstack((img, caption)))
    cv2.namedWindow('Group_analysis_results', cv2.WINDOW_KEEPRATIO)
    cv2.imshow('Group_analysis_results', np.vstack((img, caption)))
    cv2.resizeWindow('Group_analysis_results', 600, 600)
    cv2.waitKey(10000)
